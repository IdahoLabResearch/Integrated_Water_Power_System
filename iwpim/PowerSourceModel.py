  ...
  #Initialization of all variables of the class 
	...
	# Output Funtion, specifying the output to send to the dispatcher
		...
		if self.state == "supply" :
			...
			# if getSupplyType(self.name) == 'Wind': #power generated by Wind
				# self.powerGenerated = getWindPower(self.name)
			
			# if getSupplyType(self.name) == 'Biomass': #power generated by Biomass
				# self.powerGenerated = [PowerSourceCapacity(self.name)*x for x in self.powerForecastDemand]
			
			# if getSupplyType(self.name) == 'Solar': #power generated by Solar
				# self.powerGenerated = getSolarPower(self.name)   
